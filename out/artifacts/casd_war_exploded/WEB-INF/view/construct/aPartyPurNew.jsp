  ...
  <div class="layui-form-item">
    ...
    <div class="layui-inline">
      ...
      <div class="layui-input-inline">
        ...
        <input type="text" name="construct_Aparty_purchase_orderNum" id="construct_Aparty_purchase_orderNum" value="${aParty.construct_Aparty_purchase_orderNum}" readonly="readonly" class="layui-input">
      </div>
    </div>
    <div class="layui-inline">
  ...
  <div class="layui-form">
      <div class="layui-form-item">
		    ...
		    <div class="layui-inline">
		      ...
		      <div class="layui-input-inline">
		        <button class="layui-btn" type="button" onclick="btnSave();">立即提交</button>
		      </div>
		    </div>
      </div>
     </div>
...
layui.use(['table','laypage','layer','laydate','jquery'], function(){
	  ...
	  table.render({
	    elem: '#materialListID'
	    	   ,cols: [[ //标题栏
	    	   	       {field: 'construct_Aparty_purEntry_id', title: '采购id',width:'9%',type:'hidden'}
	    	   	      ...
	    	   	      ,{field: 'experience', title: '操作',width:'12%',templet: function (d) {
	    	   	    	  return  openStatus();}
	    	   	      }
	    	   	    ]]
	    ,data: []
	    ...
	    if(layEvent === 'delrow'){ //删除行
	    	...
	    	varTable = obj;
	    	 ...
	    	 layer.open({
				  ...
				  area: ['50%','50%'], 
				  maxmin: true,
				  content: 'aPartyMaterialCheck.do?construct_project_id='+$("#construct_project_id").val(),
	 ...
	 function setUserName(data){
		...
		if(userid == ''|| userid ==undefined){
			...
			$.post("save_aPartyPur.do?rows="
				+ encodeURI(JSON.stringify(data))+"&project_name="+project_name+"&userid="+userid, $("#ownhead").serialize(),
				function(data) {
					if (data.Success) {
						 layer.msg(data.Msg, {icon: 6, time: 1000,shade:[0.3]}, function () {
								location.href="aPartyPurList.do?construct_project_id="+$("#construct_project_id").val();
	                   });
					} else {
		...
		varTable.update({
			 ...
			 */
		/* 	rows[0].construct_Aparty_purEntry_materialId = data.construct_Aparty_material_id;
			rows[0].construct_Aparty_material_name = data.construct_Aparty_material_name;
			rows[0].construct_Aparty_material_model = data.construct_Aparty_material_model;
			rows[0].construct_Aparty_material_unit = data.construct_Aparty_material_unit;
			rows[0].construct_Aparty_material_num = data.construct_Aparty_material_num;
			rows[0].construct_Aparty_material_category = data.construct_Aparty_material_category; */
